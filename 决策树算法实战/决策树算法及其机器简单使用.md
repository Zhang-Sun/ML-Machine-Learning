...
同理，计算其余的特征的信息增益g(D,A2)、g(D,A3)和g(D,A4)。分别为：![](http://latex.codecogs.com/gif.latex?g(D,A_2)=H(D)-[\\frac{5}{15}H(D_1)+\frac{10}{15}H(D_2)]=0.971-[\frac{5}{15}•0+\frac{10}{15}(-\frac{4}{10}\log_2\frac{4}{10}-\frac{6}{10}\log_2\frac{6}{10})]=0.971-0.647=0.324)
...
输入：训练数据集D，特征集A，阈值![](http://latex.codecogs.com/gif.latex?\\varepsilon);
输出：决策树T；
1. 若D中所有实例均属于同一类![](http://latex.codecogs.com/gif.latex?C_k),则T为单结点树，并将![](http://latex.codecogs.com/gif.latex?C_k)作为该结点的类标记，返回T；
2. 若A=ø，则T为单结点树，并将D中实例数最大的类![](http://latex.codecogs.com/gif.latex?C_k)作为该结点的类标记，返回T；
3. 否则，计算特征对D的信息增益，选择信息增益最大的特征![](http://latex.codecogs.com/gif.latex?A_g)；
4. 若$A_g$的信息增益小于阈值![](http://latex.codecogs.com/gif.latex?\\varepsilon)，则置T为单结点树，并将D中实例数最大的类![](http://latex.codecogs.com/gif.latex?C_k)作为该结点的类标记，返回T；
5. 否则，对![](http://latex.codecogs.com/gif.latex?A_g)的每一个可能取值![](http://latex.codecogs.com/gif.latex?a_i)，依![](http://latex.codecogs.com/gif.latex?A_g=a_i)将D分隔成若干非空子集![](http://latex.codecogs.com/gif.latex?D_i)，将![](http://latex.codecogs.com/gif.latex?D_i)中实例数最大的类作为标记，构建子节点，由结点及其子节点构成树T，返回T；
6. 对第i个子节点，以Di为训练集，以![](http://latex.codecogs.com/gif.latex?A-{Ag})为特征集，递归的调用步骤1-5，得到子树Ti，返回Ti；

分析数据：

上面已经求得，特征A3（有自己的房子）的信息增益最大，所以选择A3作为根结点的特征，它将训练集D划分为两个子集D1（A3的取值是“是”）D2（A3的取值为“否”）。由于D1只有同一类的样本点，所以它成为一个叶结点，结点的类标记为“是”。

对D2则需要从特征A1（年龄），A2（有工作）和A4（信贷情况）中选择新的特征，计算各个特征的信息增益：![](http://latex.codecogs.com/gif.latex?g(D2,A1)=H(D2)-H(D2|A1)=0.251) 
![](http://latex.codecogs.com/gif.latex?g(D2,A2)=H(D2)-H(D2|A2)=0.918) 
![](http://latex.codecogs.com/gif.latex?g(D2,A4)=H(D2)-H(D2|A4)=0.474) 
...
1. 若D中所有实例均属于同一类![](http://latex.codecogs.com/gif.latex?C_k),则T为单结点树，并将![](http://latex.codecogs.com/gif.latex?C_k)作为该结点的类标记，返回T；
2. 若A=ø，则T为单结点树，并将D中实例数最大的类![](http://latex.codecogs.com/gif.latex?C_k)作为该结点的类标记，返回T；
3. 否则，计算特征对D的信息增益比，选择信息增益比最大的特征![](http://latex.codecogs.com/gif.latex?A_g)；
4. 若$A_g$的信息增益比小于阈值![](http://latex.codecogs.com/gif.latex?\\varepsilon)，则置T为单结点树，并将D中实例数最大的类![](http://latex.codecogs.com/gif.latex?C_k)作为该结点的类标记，返回T；
5. 否则，对![](http://latex.codecogs.com/gif.latex?A_g)的每一个可能取值![](http://latex.codecogs.com/gif.latex?a_i)，依![](http://latex.codecogs.com/gif.latex?A_g=a_i)将D分隔成若干非空子集![](http://latex.codecogs.com/gif.latex?D_i)，将![](http://latex.codecogs.com/gif.latex?D_i)中实例数最大的类作为标记，构建子节点，由结点及其子节点构成树T，返回T；
6. 对第i个子节点，以Di为训练集，以![](http://latex.codecogs.com/gif.latex?A-{Ag})为特征集，递归的调用步骤1-5，得到子树Ti，返回Ti；
递归构造决策树：  
