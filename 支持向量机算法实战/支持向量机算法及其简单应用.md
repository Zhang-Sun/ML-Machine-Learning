...
<html>
    <img src='LinearSVM1.png' width=300 height=200>
</html>
...
由数学知识：平面外的点x到平面的距离是：$$d=\frac{|w^T+b|}{||w||}$$

这就是几何间隔的定义。

这个公式是如下公式的变种：$$d=\frac{|Ax+By+C|}{\sqrt{A^2+B^2}}$$
...
需要特别注意的是，如果找到了一个超平面可以正确分开这个数据集，那么意味着对于每个样本点x来说，其实它的函数间隔都大于0，即：$$y_i(w^T+b)>0$$

解释如下：

对于正样例，它的yi=+1，而因为它被正确分类了，所以$$w^T+b>0$$所以可以得到$$y_i(w^T+b)>0$$对于负样例同理。

进一步，我们可以得到，如果这个样本被正确分类了，那么 (加绝对值不影响，yi=+-1)$$y_i(w^T+b)=|w^T+b|$$

所以函数间隔其实也可以用$|w^T+b|$表示。

而几何间隔为$\frac{|w^T+b|}{||w||}$，很多时候分子都不用绝对值来表示，我们可以把分子的绝对值换为函数间隔，那么这个样本到超平面的几何间隔其实就写成了$$\frac{y_i(w^Tx_i+b)}{||w||}$$
...
<html>
    <img src='LinearSVM3.png' width=300 height=300>
</html>

最大化$\frac{2}{||w||}$即为最小化$||w||^2$

<html>
    <img src='LinearSVM4.png' width=300 height=300>
    </html>
...
对yi=+1的正例点，支持向量在超平面：$$H_1:w·x_i+b=1$$
...
<html>
    <img src='LinearSVM1.png' width=300 height=300>
</html>

**间隔边界**

H1和H2平行，且没有实例点落在他们中间，分离超平面与它们平行位于中央，间隔即为H1和H2的直接距离，依赖于分离超平面的法向量w，等于$\frac{2}{||w||}$，H1和H2称为间隔边界。
...
$$s.t.y_i(w·x_i+b)-1\geqslant0,i=1,2,...,N$$
...
$$y_i(w·x_i+b)\geqslant1-\xi_i$$

同时对每个松弛变量$\xi_i$，支付一个代价$\xi_i$，目标函数就变成了

$$\frac{1}{2}||w||^2+C\sum_{i=1}^N\xi_i$$

这里C>0为惩罚参数，一般由应用问题决定，C值大时对误分类的惩罚加大。这时，最小化目标函数就包含两层含义：（1）使$\frac{1}{2}||w||^2$尽量小即间隔仅两大，同时使误分类点的个数尽量小，C是调和二者的系数。
...
$$s.t.y_i(w·x_i+b)\geqslant1-\xi_i$$

$$\xi_i\geqslant0,i=1,2,...,N$$

这是一个凸二次规划问题，因而关于$(w,b,\xi)$的解是存在的，并且可以证明w的解是唯一的，但b不唯一，b的解存在于一个区间。

以上求解模型就为线性不可分支持向量机模型。

<html>
    <img src='SVM5.png' width=500 height=300>
</html>

<html>
    <img src='SVM6.png' width=500 height=300>
</html>
...
<html>
    <img src='SVM7.png' width=500 height=300>
</html>

<html>
    <img src='SVM8.png' width=500 height=300>
</html>

<html>
    <img src='SVM9.png' width=500 height=300>
</html>

<html>
    <img src='SVM10.png' width=500 height=300>
</html>

在步骤（2）中，对任一适合条件0<$a^*_j$<C的$a^*$都可以求出$b^*$，但是由于问题（7.32）～（7.34）对b的解不唯一，所以实际计算时可以取在所有符合条件的样本点上的平均值。

### 5.2.3 支持向量

<html>
    <img src='SVM11.png' width=500 height=300>
</html>

### 5.2.4 合叶损失函数

<html>
    <img src='SVM12.png' width=500 height=300>
</html>

<html>
    <img src='SVM13.png' width=500 height=300>
</html>

<html>
    <img src='SVM14.png' width=500 height=300>
</html>

<html>
    <img src='SVM15.png' width=500 height=300>
</html>
...
<html>
    <img src='SVM16.png' width=500 height=300>
</html>
...
<html>
    <img src='SVM17.png' width=700 height=500>
</html>
...
<html>
    <img src='SVM18.png' width=500 height=300>
    <img src='SVM19.png' width=500 height=300>
</html>
...
<html>
    <img src='SVM20.png' width=500 height=300>
</html>

### 5.3.3常用的核函数

<html>
    <img src='SVM21.png' width=500 height=300>
    </html>
...
<html>
    <img src='SVM22.png' width=300 height=300>
</html>

**非线性支持向量机学习算法：**

<html>
    <img src='SVM23.png' width=500 height=300>
</html>
...
<html>
    <img src='SVM24.png' width=500 height=300>
</html>
...
<html>
    <img src='SVM25.png' width=700 height=500>
</html>
...
```
      ...
      File "<ipython-input-1-88216251d18a>", line 1
        class sklearn.svm.SVC(C=1.0, kernel=’rbf’, degree=3, gamma=’auto’, coef0=0.0, shrinking=True, probability=False, tol=0.001, cache_size=200, class_weight=None, verbose=False, max_iter=-1, decision_function_shape=’ovr’, random_state=None)
                     ^
